"""The five gates. Every one is shown passing AND shown failing.

A gate nobody has watched fail is a gate nobody should believe. So each of the
five is broken deliberately here, once, and asserted red. Nothing in this file
...
def test_validate_runs_all_five_gates():
    ledger = _ledger(load("L-1", 1000.0))
    gates = validate(ledger, [])

    assert len(gates) == 5
    assert [g.rule.split(":")[0] for g in gates] == ["G1", "G2", "G3", "G4", "G5"]
...
def test_a_clean_month_passes_every_gate():
    ...
    assert summary(gates) == "5/5 gates passed"
